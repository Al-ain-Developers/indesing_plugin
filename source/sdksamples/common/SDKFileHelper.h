//========================================================================================
//  
//  $File: //depot/devtech/16.0.x/plugin/source/sdksamples/common/SDKFileHelper.h $
//  
...
public:
	...
	*/
	...
	/** Get the selected index of fileter combo box.
		@return the selected index of fileter control, or -1.
	*/
...
/**	Provides a mechanism for the user to choose multiple files to be opened and allows
	filtering of the files presented, note this class does not actually open the
	file, it just lets the user choose a file.
	...
	multiFileChooser.AddFilter(kGIFFileTypeInfoID, "GIFf files(gif)");
	...
	@ingroup sdk_common
 */
class SDKFileOpenMultipleFileChooser : public SDKFileOpenChooser {

public:
	
	/**	Pops a file browse dialog to allow the user to choose multiple files to be opened. If any filters have been
		specified by the caller (SDKFileOpenChooser::AddFilter, SDKFileOpenChooser::AddAllFiles), the
		user will be able to apply them to control the files shown in the chooser.
		Otherwise two default filters will be made available; the first filter will show
		publication documents (InDesign documents under InDesign, InCopy documents under InCopy);
